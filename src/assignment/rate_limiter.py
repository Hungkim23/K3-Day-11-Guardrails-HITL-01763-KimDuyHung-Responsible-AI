...
class RateLimitPlugin(base_plugin.BasePlugin):
    ...
    async def on_user_message_callback(self, *, invocation_context, user_message):
        ...
        # 2. Check if user exceeded the request limit
        if len(window) >= self.max_requests:
            wait = self.window_seconds - (now - window[0])
            self.blocked_count += 1
            return self._block_response(
                f"Rate limit exceeded. You can send at most {self.max_requests} "
                f"requests per {self.window_seconds}s. "
                f"Please try again in {wait:.0f}s."
            )

        # 3. Within limit — record this request and allow through
        window.append(now)
        return None
